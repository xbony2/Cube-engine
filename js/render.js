...
Renderer.prototype.lockPointer = function(){
	...
	if('mozPointerLockElement' in document){
		alert("Firefox needs full screen to lock mouse. Use Chrome or something.")
		return
	}
...
Renderer.prototype.getFrustrumPlanes = function(){
	...
	for(var i = 0; i < 4; i++){
		...
		if(!length)
			length = 1 / Math.sqrt(this.frustrum[i].x * this.frustrum[i].x + 
					this.frustrum[i].y * this.frustrum[i].y + this.frustrum[i].z * this.frustrum[i].z)
...
Renderer.prototype.render = function(){
	...
	if(this.mouseClick){
		...
		}else if(this.clickedNode && this.mouseClick.button == 2) // right click = remove node
	...
	if(this.mouselock)
		this.context.drawImage(this.crosshair, this.w2 - 8, this.h2 - 8) // render crosshair
...
Renderer.prototype.drawRect = function(p){
	...
	if(this.mouseClick)
		if((this.mouseClick.y - this.rp[p[0]].y) * (this.rp[p[1]].x - this.rp[p[0]].x) - 
...
Renderer.prototype.drawMonochrome = function(p){	
	...
	if(points.length > 1){
		...
		// fill doesn't work properly in Chrome 20.0.1132.57 //who cares -xbony2
		this.context.fill()
		this.context.stroke()
		this.context.globalAlpha = 1
	}
...
Renderer.prototype.displayHud = function(){
	...
	this.context.fillText("Version: 2.0.0a", 0, 96)
	this.context.restore()
}
