...
Player.prototype.collisionDetection = function(){
	...
	for(var i in this.collisionNodes){
		...
		// collision on x axis
		if(this.delta.x && this.position.z + this.size > node.z && 
		...
		// collision on z axis
		if(this.delta.z && this.position.x + this.size > node.x &&
...
Player.prototype.nodeCollision = function(node){
	if(this.position.x+this.size > node.x && this.position.x-this.size < node.x+1 &&
			this.position.z+this.size > node.z && this.position.z-this.size < node.z+1 &&
			this.position.y+0.2 > node.y && this.position.y < node.y+1)
		return true
	return false
}
